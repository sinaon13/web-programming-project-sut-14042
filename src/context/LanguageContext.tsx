'use client';
import React, { createContext, useContext, useState, useEffect } from 'react';
import { authAPI, getAccessToken } from '@/lib/api';

type Language = 'en' | 'fa';

export const dictionary = {
  en: {
    // Navigation & Sidebar
    home: 'Home',
    browse: 'Browse All Music',
    albums: '💿 Albums Archive',
    singles: '🎵 Singles Archive',
    playlists: 'My Playlists',
    profile: 'My Profile',
    tickets: '🎧 Support Tickets',
    notifications: 'Notifications',
    settings: 'Settings',
    logout: 'Log Out of Account',
    artistStudio: 'Artist Studio',
    adminDashboard: 'Admin Dashboard',
    
    // Player & Queue
    shuffle: 'SHUFFLE',
    rep: 'REP',
    lyrics: 'Lyrics',
    vol: 'Vol',
    queue: 'Queue',
    upNext: 'Up Next in Queue',
    emptyQueue: 'Your queue is currently empty.',
    closeLyrics: 'Close Lyrics',
    play: 'Play',
    expandPlayer: '⛶ Full Screen',
    collapsePlayer: '▼ Close Full Screen',
    nowPlaying: 'Now Playing in Full Screen',
    byArtist: 'By',
    crossfadeOn: 'ON',
    crossfadeOff: 'OFF',
    qualityLabel: 'Quality:',
    
    // Home Page
    welcomeBack: 'Welcome back',
    discoverText: 'Discover top trending albums and tracks on the platform.',
    recentPlaylists: '🎧 Recently Played Playlists',
    tracksCount: 'tracks',
    vipExclusive: '⭐ VIP Gold Exclusive: Early Access Releases',
    premiumFeature: 'Premium Feature',
    playVip: 'Play VIP',
    popularReleases: 'Popular Releases',
    playTrack: 'Play Track',
    featuredAlbums: 'Featured Albums (Click to View Tracklist)',

    // Browse Page
    searchPlaceholder: 'Search by song title or artist name...',
    sortBy: 'Sort By:',
    mostListeners: 'Most Listeners',
    releaseDate: 'Release Date',
    streams: 'streams',
    unique: 'unique',
    unlockVip: '🔒 Unlock VIP',

    // Albums & Singles Archives
    albumsArchiveTitle: 'Albums Archive',
    albumsArchiveDesc: 'Explore full discography releases from verified platform artists.',
    singlesArchiveTitle: 'Singles Archive',
    singlesArchiveDesc: 'Stream standalone single tracks released directly by artists.',
    playSingle: 'Play Single',

    // Playlists Page
    playlistsTitle: 'My Playlists',
    currentTierLabel: 'Current Tier:',
    newPlaylistPlaceholder: 'New playlist name...',
    createBtn: 'Create',
    noPlaylists: 'No playlists found. Create your first playlist above!',
    renameBtn: '✏️ Rename',
    deleteBtn: 'Delete',
    noSongsInList: 'No songs in this list yet.',
    addSongsFromArchive: '➕ Add Songs from Archive',
    playlistLimitReached: 'Playlist limit reached for your tier!',
    playlistCreated: 'Playlist created!',
    playlistDeleted: 'Playlist deleted.',
    playlistRenamed: 'Playlist renamed.',
    trackRemovedFromPlaylist: 'Track removed from playlist.',
    playlistAddTrackFailed: 'Failed to modify playlist.',

    // Notifications Page
    sysNotificationsTitle: 'System Notifications',
    markAllRead: 'Mark All as Read',
    noNotifs: 'No system notifications.',
    markReadBtn: '✓ Mark Read',
    deleteNotifBtn: 'Delete',
    viewDetails: 'View Details ➡️',

    // Support Tickets Page
    supportTicketsTitle: 'Submit New Support Ticket',
    supportTicketsDesc: 'Report playback errors, verification inquiries, or billing questions.',
    issueSubject: 'Issue Subject',
    issueSubjectPlaceholder: 'e.g., Audio buffering error...',
    msgDesc: 'Message Description',
    msgDescPlaceholder: 'Describe your issue in detail...',
    submitTicketBtn: 'Submit Ticket',
    myTicketHistory: 'My Ticket History',
    noTickets: 'No support tickets created yet.',
    sentAt: 'Sent:',
    ticketSubmitted: 'Support ticket submitted successfully! Staff has been notified.',
    replyPlaceholder: 'Type your reply...',

    // Settings Page
    subManagement: 'Subscription Tier Management',
    currentTier: 'Current Active Tier',
    silverPlan: 'Silver Plan',
    goldPlan: 'VIP Gold Plan',
    selectSilver: 'Select Silver',
    upgradeGold: 'Upgrade Gold',
    preferences: 'Platform & Audio Preferences',
    sysVolume: 'Default System Sound Volume',
    volDesc: 'Sets the baseline volume across all streaming media and players.',
    notifLimits: 'Notification Limitations',
    notifReleases: 'Receive alerts for new track releases from followed artists',
    notifExpire: 'Receive subscription expiration & renewal warnings',
    notifEmail: 'Send system notifications directly to my registered email',
    interfaceLang: 'Interface Language',
    savePrefs: 'Save All Preferences',
    deleteAccountBtn: 'Delete Account & All Data',
    deleteAccountConfirm: '⚠️ WARNING: This will permanently delete your account, playlists, tickets, and all uploaded works! Proceed?',
    planActivated: 'Subscription plan activated!',
    prefsSaved: 'Platform preferences and notification settings saved!',
    accountDeleted: 'Account and all associated data deleted permanently.',

    // Artist Portal & Profile
    verifiedBadge: '✓ Verified Artist',
    artistStudioTitle: 'Artist Studio & Upload Center',
    bioSettings: 'Artist Biography Settings',
    bioPlaceholder: 'Tell listeners about your musical background...',
    saveBio: 'Save Biography',
    trackTitle: 'Track Title',
    releaseType: 'Release Type',
    singleOption: 'Single',
    albumOption: 'Album Track',
    audioFormat: 'Audio File Format',
    genre: 'Genre',
    releaseYear: 'Release Year',
    collaborators: 'Collaborating Artists',
    albumName: 'Album Name',
    coverUrlLabel: 'Cover Image URL (Optional Custom Artwork)',
    coverUrlPlaceholder: 'https://images.unsplash.com/...',
    publishBtn: 'Publish Track to Platform',
    updateBtn: 'Update Published Track ➡️',
    publishedWorks: 'Published Works Management & Revenue Analytics',
    colTitle: 'Title',
    colFormat: 'Format',
    colListeners: 'Listeners',
    colStreams: 'Total Streams',
    colRevenue: 'Est. Revenue (IRR)',
    colActions: 'Actions',
    editBtn: '✏️ Edit',
    cancelEdit: 'Cancel Edit',
    followArtist: 'Follow Artist',
    followingArtist: 'Unfollow',
    aboutBio: 'About / Biography',
    releasedAlbums: 'Released Albums',
    completeDiscography: 'Complete Discography',
    savePriceChanges: 'Save Price Changes',
    unsettled: 'Unsettled Streams',
    trackPublished: 'Track published to the platform!',
    trackUpdated: 'Track updated successfully!',
    trackDeleted: 'Track deleted from the platform.',
    bioUpdated: 'Biography updated successfully!',
    titleRequired: 'Title is required.',
    audioRequired: 'Audio file is required for new tracks.',
    artistRejectedTitle: '⛔ Artist Application Rejected',
    artistPendingTitle: '⏳ Application Under Review',
    earlyAccessLabel: 'Early Access (VIP Only)',
    coverImageLabel: 'Cover Image',
    audioFileLabel: 'Audio File',

    // Admin Panel
    verificationsTab: 'Verifications',
    ticketsTab: 'Support Tickets',
    accountingTab: 'Financial Accounting',
    pricingTab: 'Revenue Charts & Pricing',
    approveBtn: 'Approve',
    rejectBtn: 'Reject',
    typeReply: 'Type staff reply...',
    sendReply: 'Send Reply',
    closeTicket: '🔒 Close Ticket',
    closedTicketMsg: 'This support ticket has been closed by staff.',
    confirmSettlement: 'Confirm Settlement',
    inPending: 'In Pending',
    settled: 'Settled',
    artistApproved: 'Artist approved! Notification sent.',
    artistRejected: 'Artist rejected! Notification sent.',
    replyFailed: 'Failed to send reply.',
    payoutSettled: 'Artist payout marked as settled!',
    pricingUpdated: 'System pricing adjusted successfully!',
    ticketClosed: 'Ticket closed successfully.',
    noPendingApps: 'No pending applications.',
    monthlyAccounting: 'Monthly Financial Accounting & Artist Payouts',
    artistNameId: 'Artist Name & ID',
    totalRegisteredStreams: 'Total Registered Streams',
    unpaidStreams: 'Unpaid Streams',
    calculatedPayout: 'Calculated Payout',
    monetization: 'Monetization',
    actionAdminOnly: 'Action (Admin Only)',
    enabled: 'Enabled',
    disabled: 'Disabled',
    revokeMonetization: 'Revoke Monetization',
    confirmMonetization: 'Confirm Monetization',
    totalSubRevenue: 'Total Subscription Revenue',
    activeGoldVip: 'Active Gold VIP Users',
    activeSilverUsers: 'Active Silver Users',
    subscribers: 'Subscribers',
    tierDistribution: 'User Subscription Tier Distribution',
    dynamicPricing: 'Dynamic Pricing Controls',
    silverTierPrice: 'Silver Tier Price (IRR)',
    goldTierPrice: 'Gold VIP Tier Price (IRR)',
    timeTravelTitle: 'Time Travel (Testing & Debug)',
    timeTravelDesc: 'Advance the global backend clock to test subscription expirations. Subscriptions that pass their expiration date will be automatically deactivated and users will be notified.',
    advanceDays: 'Advance',
    days: 'Days',
    accessDenied: 'Access Denied',

    // Login & Register
    loginTitle: 'Log in to Spotify Clone',
    emailLabel: 'Email Address',
    passwordLabel: 'Password',
    confirmPasswordLabel: 'Confirm Password',
    signingIn: 'Signing in...',
    signIn: 'Sign In',
    forgotPassword: 'Forgot Password?',
    createAccount: 'Create Account',
    resetPassword: 'Reset Password',
    resetPasswordDesc: 'Enter your email to receive recovery instructions.',
    sendRecoveryLink: 'Send Recovery Link',
    sending: 'Sending...',
    cancel: 'Cancel',
    recoverySent: 'Recovery email sent!',
    recoveryFailed: 'Failed to send recovery email.',
    listenerAccount: 'Listener Account',
    artistApplication: 'Artist Application',
    stageArtistName: 'Stage / Artist Name',
    displayName: 'Display Name',
    birthDate: 'Birth Date',
    genderLabel: 'Gender',
    male: 'Male',
    female: 'Female',
    other: 'Other',
    portfolioUrl: 'Portfolio Sample URL',
    privacyPolicyAgree: 'I agree to the',
    privacyPolicy: 'Privacy Policy',
    creatingAccount: 'Creating Account...',
    alreadyHaveAccount: 'Already have an account?',
    privacyPolicyTitle: 'Platform Privacy Policy',
    privacyPolicyText: 'We collect basic account details and stream telemetry to calculate fair artist payouts. We never share your data with unauthorized third parties.',
    closeReturn: 'Close & Return',
    passwordMinLength: 'Password must be at least 8 characters long.',
    passwordMismatch: 'Passwords do not match.',
    acceptPolicy: 'You must accept the Privacy Policy to proceed.',
    minChars: 'Min. 8 characters',

    // General
    backendOffline: 'Backend offline.',
    unknownError: 'Unknown error',
    downloadBasicRestricted: 'Song downloading is strictly exclusive to Silver and Gold VIP subscribers! Would you like to upgrade your tier now?',
    downloadFailed: 'Download failed',
    priority: 'Priority',
    assignedTo: 'Assigned:',
    reply: 'Reply',
    by: 'by',
  },
  fa: {
    // Navigation & Sidebar
    home: 'خانه',
    browse: 'جستجوی موسیقی',
    albums: '💿 آرشیو آلبوم‌ها',
    singles: '🎵 آرشیو تک‌آهنگ‌ها',
    playlists: 'پلی‌لیست‌های من',
    profile: 'نمایه کاربری',
    tickets: '🎧 تیکت‌های پشتیبانی',
    notifications: 'اعلانات',
    settings: 'تنظیمات برنامه',
    logout: 'خروج از حساب کاربری',
    artistStudio: 'پنل هنرمندان',
    adminDashboard: 'داشبورد مدیریت',
    
    // Player & Queue
    shuffle: 'تصادفی',
    rep: 'تکرار',
    lyrics: 'متن آهنگ',
    vol: 'صدا',
    queue: 'صف پخش',
    upNext: 'آهنگ‌های بعدی در صف',
    emptyQueue: 'صف پخش شما در حال حاضر خالی است.',
    closeLyrics: 'بستن متن آهنگ',
    play: 'پخش',
    expandPlayer: '⛶ تمام‌صفحه',
    collapsePlayer: '▼ بستن حالت تمام‌صفحه',
    nowPlaying: 'در حال پخش در حالت تمام‌صفحه',
    byArtist: 'توسط',
    crossfadeOn: 'روشن',
    crossfadeOff: 'خاموش',
    qualityLabel: 'کیفیت:',
    
    // Home Page
    welcomeBack: 'خوش آمدید',
    discoverText: 'جدیدترین و محبوب‌ترین آثار موسیقی را در سامانه کشف کنید.',
    recentPlaylists: '🎧 آخرین پلی‌لیست‌های شنیده‌شده',
    tracksCount: 'آهنگ',
    vipExclusive: '⭐ دسترسی زودهنگام ویژه مشترکین طلایی',
    premiumFeature: 'امکانات ویژه',
    playVip: 'پخش ویژه',
    popularReleases: 'آثار پرشنونده',
    playTrack: 'پخش آهنگ',
    featuredAlbums: 'آلبوم‌های برگزیده (برای مشاهده لیست آهنگ‌ها کلیک کنید)',

    // Browse Page
    searchPlaceholder: 'جستجو بر اساس نام اثر یا هنرمند...',
    sortBy: 'مرتب‌سازی بر اساس:',
    mostListeners: 'بیشترین شنونده',
    releaseDate: 'تاریخ انتشار',
    streams: 'استریم',
    unique: 'شنونده یکتا',
    unlockVip: '🔒 ارتقا به طلایی',

    // Albums & Singles Archives
    albumsArchiveTitle: 'آرشیو آلبوم‌ها',
    albumsArchiveDesc: 'آثار و آلبوم‌های منتشرشده از هنرمندان تاییدشده سامانه را کاوش کنید.',
    singlesArchiveTitle: 'آرشیو تک‌آهنگ‌ها',
    singlesArchiveDesc: 'تک‌آهنگ‌های منتشرشده توسط هنرمندان را گوش دهید.',
    playSingle: 'پخش تک‌آهنگ',

    // Playlists Page
    playlistsTitle: 'پلی‌لیست‌های من',
    currentTierLabel: 'سطح اشتراک:',
    newPlaylistPlaceholder: 'نام پلی‌لیست جدید...',
    createBtn: 'ایجاد',
    noPlaylists: 'هیچ پلی‌لیستی یافت نشد. اولین پلی‌لیست خود را در بالا بسازید!',
    renameBtn: '✏️ تغییر نام',
    deleteBtn: 'حذف',
    noSongsInList: 'هنوز آهنگی به این لیست اضافه نشده است.',
    addSongsFromArchive: '➕ افزودن آهنگ از آرشیو',
    playlistLimitReached: 'تعداد پلی‌لیست‌ها به حداکثر مجاز رسیده است!',
    playlistCreated: 'پلی‌لیست ایجاد شد!',
    playlistDeleted: 'پلی‌لیست حذف شد.',
    playlistRenamed: 'نام پلی‌لیست تغییر کرد.',
    trackRemovedFromPlaylist: 'آهنگ از پلی‌لیست حذف شد.',
    playlistAddTrackFailed: 'خطا در تغییر پلی‌لیست.',

    // Notifications Page
    sysNotificationsTitle: 'اعلانات سیستمی',
    markAllRead: 'علامت‌گذاری همه به عنوان خوانده‌شده',
    noNotifs: 'هیچ اعلان سیستمی وجود ندارد.',
    markReadBtn: '✓ خوانده‌شد',
    deleteNotifBtn: 'حذف',
    viewDetails: 'مشاهده جزئیات ⬅️',

    // Support Tickets Page
    supportTicketsTitle: 'ثبت تیکت پشتیبانی جدید',
    supportTicketsDesc: 'گزارش خطاهای پخش، سوالات احراز هویت یا امور مالی.',
    issueSubject: 'موضوع تیکت',
    issueSubjectPlaceholder: 'مثلا: مشکل در پخش آهنگ...',
    msgDesc: 'متن پیام',
    msgDescPlaceholder: 'مشکل خود را با جزئیات شرح دهید...',
    submitTicketBtn: 'ارسال تیکت',
    myTicketHistory: 'تاریخچه تیکت‌های من',
    noTickets: 'هنوز تیکتی ثبت نکرده‌اید.',
    sentAt: 'ارسال:',
    ticketSubmitted: 'تیکت پشتیبانی شما با موفقیت ارسال شد! تیم پشتیبانی مطلع شده است.',
    replyPlaceholder: 'پاسخ خود را بنویسید...',

    // Settings Page
    subManagement: 'مدیریت اشتراک و سطح دسترسی',
    currentTier: 'سطح اشتراک فعال شما',
    silverPlan: 'اشتراک نقره‌ای',
    goldPlan: 'اشتراک ویژه طلایی',
    selectSilver: 'انتخاب نقره‌ای',
    upgradeGold: 'ارتقا به طلایی',
    preferences: 'تنظیمات پلتفرم و پخش صدا',
    sysVolume: 'میزان صدای پیش‌فرض سامانه',
    volDesc: 'میزان صدای اولیه را برای تمامی پخش‌کننده‌ها و آهنگ‌ها تنظیم می‌کند.',
    notifLimits: 'محدودیت و مدیریت اعلانات',
    notifReleases: 'دریافت هشدار انتشار آثار جدید از هنرمندان دنبال‌شده',
    notifExpire: 'دریافت هشدار اتمام مهلت و تمدید اشتراک',
    notifEmail: 'ارسال اعلانات سیستمی به ایمیل ثبت‌شده من',
    interfaceLang: 'زبان رابط کاربری',
    savePrefs: 'ذخیره تمامی تنظیمات',
    deleteAccountBtn: 'حذف کامل حساب کاربری و تمامی داده‌ها',
    deleteAccountConfirm: '⚠️ هشدار: با این کار حساب کاربری، پلی‌لیست‌ها، تیکت‌ها و تمامی آثار منتشرشده شما برای همیشه حذف خواهند شد! ادامه می‌دهید؟',
    planActivated: 'اشتراک فعال شد!',
    prefsSaved: 'تنظیمات پلتفرم و اعلانات ذخیره شد!',
    accountDeleted: 'حساب کاربری و تمامی داده‌ها برای همیشه حذف شدند.',

    // Artist Portal & Profile
    verifiedBadge: '✓ نشان هنرمند تایید شده',
    artistStudioTitle: 'استودیو و پنل مدیریت آثار هنرمندان',
    bioSettings: 'تنظیمات بیوگرافی و معرفی هنرمند',
    bioPlaceholder: 'درباره سوابق و فعالیت‌های موسیقی خود برای شنوندگان بنویسید...',
    saveBio: 'ذخیره بیوگرافی',
    trackTitle: 'نام اثر / آهنگ',
    releaseType: 'نوع انتشار',
    singleOption: 'تک‌آهنگ (Single)',
    albumOption: 'قطعه‌ای از آلبوم (Album Track)',
    audioFormat: 'فرمت فایل صوتی',
    genre: 'ژانر موسیقی',
    releaseYear: 'سال انتشار',
    collaborators: 'هنرمندان همکار',
    albumName: 'نام آلبوم موسیقی',
    coverUrlLabel: 'آدرس اینترنتی تصویر کاور (اختیاری جهت شخصی‌سازی)',
    coverUrlPlaceholder: 'https://images.unsplash.com/...',
    publishBtn: 'انتشار اثر در سامانه',
    updateBtn: 'بروزرسانی اثر منتشرشده ⬅️',
    publishedWorks: 'مدیریت آثار منتشرشده و آمار درآمدی',
    colTitle: 'نام اثر',
    colFormat: 'فرمت',
    colListeners: 'شنوندگان',
    colStreams: 'مجموع استریم‌ها',
    colRevenue: 'درآمد برآوردشده (تومان)',
    colActions: 'عملیات',
    editBtn: '✏️ ویرایش',
    cancelEdit: 'لغو ویرایش',
    editProfile: 'ویرایش نمایه',
    followArtist: 'دنبال کردن هنرمند',
    followingArtist: 'لغو دنبال کردن',
    aboutBio: 'درباره / بیوگرافی',
    releasedAlbums: 'آلبوم‌های منتشر شده',
    completeDiscography: 'دیسکوگرافی کامل',
    savePriceChanges: 'ذخیره تغییرات قیمت',
    unsettled: 'استریم‌های تسویه‌نشده',
    trackPublished: 'اثر با موفقیت در سامانه منتشر شد!',
    trackUpdated: 'اثر با موفقیت بروزرسانی شد!',
    trackDeleted: 'اثر از سامانه حذف شد.',
    bioUpdated: 'بیوگرافی با موفقیت بروزرسانی شد!',
    titleRequired: 'وارد کردن نام اثر الزامی است.',
    audioRequired: 'فایل صوتی برای آثار جدید الزامی است.',
    artistRejectedTitle: '⛔ درخواست هنرمندی رد شد',
    artistPendingTitle: '⏳ درخواست در حال بررسی',
    earlyAccessLabel: 'دسترسی زودهنگام (فقط VIP)',
    coverImageLabel: 'تصویر کاور',
    audioFileLabel: 'فایل صوتی',

    // Admin Panel
    verificationsTab: 'تایید هنرمندان',
    ticketsTab: 'تیکت‌های پشتیبانی',
    accountingTab: 'حسابرسی مالی',
    pricingTab: 'قیمت‌گذاری و نمودار درآمد',
    approveBtn: 'تایید',
    rejectBtn: 'رد',
    typeReply: 'تایپ پاسخ پشتیبان...',
    sendReply: 'ارسال پاسخ',
    closeTicket: '🔒 بستن تیکت',
    closedTicketMsg: 'این تیکت توسط تیم پشتیبانی بسته شده است.',
    confirmSettlement: 'تایید تسویه حساب',
    inPending: 'در انتظار پرداخت',
    settled: 'تسویه شده',
    artistApproved: 'هنرمند تایید شد! اعلان ارسال شد.',
    artistRejected: 'هنرمند رد شد! اعلان ارسال شد.',
    replyFailed: 'ارسال پاسخ با خطا مواجه شد.',
    payoutSettled: 'پرداخت هنرمند انجام شد!',
    pricingUpdated: 'قیمت‌گذاری سامانه با موفقیت تغییر کرد!',
    ticketClosed: 'تیکت با موفقیت بسته شد.',
    noPendingApps: 'هیچ درخواست معلقی وجود ندارد.',
    monthlyAccounting: 'حسابرسی مالی ماهانه و پرداخت هنرمندان',
    artistNameId: 'نام و شناسه هنرمند',
    totalRegisteredStreams: 'مجموع استریم‌های ثبت‌شده',
    unpaidStreams: 'استریم‌های پرداخت‌نشده',
    calculatedPayout: 'مبلغ قابل پرداخت',
    monetization: 'درآمدزایی',
    actionAdminOnly: 'عملیات (فقط مدیر)',
    enabled: 'فعال',
    disabled: 'غیرفعال',
    revokeMonetization: 'لغو درآمدزایی',
    confirmMonetization: 'تایید درآمدزایی',
    totalSubRevenue: 'مجموع درآمد اشتراک‌ها',
    activeGoldVip: 'کاربران فعال طلایی',
    activeSilverUsers: 'کاربران فعال نقره‌ای',
    subscribers: 'مشترک',
    tierDistribution: 'توزیع سطوح اشتراک کاربران',
    dynamicPricing: 'کنترل‌های قیمت‌گذاری پویا',
    silverTierPrice: 'قیمت اشتراک نقره‌ای (ریال)',
    goldTierPrice: 'قیمت اشتراک طلایی (ریال)',
    timeTravelTitle: 'سفر در زمان (تست و دیباگ)',
    timeTravelDesc: 'ساعت سمت سرور را جلو ببرید تا انقضای اشتراک‌ها تست شود. اشتراک‌هایی که از تاریخ انقضایشان گذشته باشد به صورت خودکار غیرفعال می‌شوند.',
    advanceDays: 'جلو بردن',
    days: 'روز',
    accessDenied: 'دسترسی غیرمجاز',

    // Login & Register
    loginTitle: 'ورود به سامانه موسیقی',
    emailLabel: 'آدرس ایمیل',
    passwordLabel: 'رمز عبور',
    confirmPasswordLabel: 'تکرار رمز عبور',
    signingIn: 'در حال ورود...',
    signIn: 'ورود به حساب',
    forgotPassword: 'فراموشی رمز عبور؟',
    createAccount: 'ایجاد حساب کاربری',
    resetPassword: 'بازیابی رمز عبور',
    resetPasswordDesc: 'ایمیل خود را وارد کنید تا دستورالعمل بازیابی ارسال شود.',
    sendRecoveryLink: 'ارسال لینک بازیابی',
    sending: 'در حال ارسال...',
    cancel: 'لغو',
    recoverySent: 'ایمیل بازیابی ارسال شد!',
    recoveryFailed: 'ارسال ایمیل بازیابی با خطا مواجه شد.',
    listenerAccount: 'حساب شنونده',
    artistApplication: 'درخواست هنرمندی',
    stageArtistName: 'نام هنری / استیج',
    displayName: 'نام نمایشی',
    birthDate: 'تاریخ تولد',
    genderLabel: 'جنسیت',
    male: 'مرد',
    female: 'زن',
    other: 'سایر',
    portfolioUrl: 'آدرس نمونه‌کار',
    privacyPolicyAgree: 'موافقت می‌کنم با',
    privacyPolicy: 'سیاست حفظ حریم خصوصی',
    creatingAccount: 'در حال ایجاد حساب...',
    alreadyHaveAccount: 'حساب کاربری دارید؟',
    privacyPolicyTitle: 'سیاست حفظ حریم خصوصی سامانه',
    privacyPolicyText: 'ما اطلاعات پایه حساب و آمار استریم را برای محاسبه عادلانه پرداخت هنرمندان جمع‌آوری می‌کنیم. داده‌های شما هرگز با اشخاص ثالث غیرمجاز به اشتراک گذاشته نمی‌شود.',
    closeReturn: 'بستن و بازگشت',
    passwordMinLength: 'رمز عبور باید حداقل ۸ کاراکتر باشد.',
    passwordMismatch: 'رمزهای عبور مطابقت ندارند.',
    acceptPolicy: 'برای ادامه باید سیاست حریم خصوصی را بپذیرید.',
    minChars: 'حداقل ۸ کاراکتر',

    // General
    backendOffline: 'سرور در دسترس نیست.',
    unknownError: 'خطای نامشخص',
    downloadBasicRestricted: 'دانلود آهنگ‌ها به صورت انحصاری فقط برای مشترکین ویژه نقره‌ای و طلایی امکان‌پذیر است! مایلید اشتراک خود را ارتقا دهید؟',
    downloadFailed: 'دانلود با خطا مواجه شد',
    priority: 'اولویت',
    assignedTo: 'تخصیص:',
    reply: 'پاسخ',
    by: 'توسط',
  }
};

interface LanguageContextType {
  language: Language;
  setLanguage: (lang: Language) => void;
  t: typeof dictionary.en;
  isRtl: boolean;
}

const LanguageContext = createContext<LanguageContextType | null>(null);

export const LanguageProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [language, setLanguageState] = useState<Language>('en');

  useEffect(() => {
    const saved = localStorage.getItem('app_lang') as Language;
    if (saved === 'fa' || saved === 'en') {
      setLanguageState(saved);
      document.documentElement.dir = saved === 'fa' ? 'rtl' : 'ltr';
      document.documentElement.lang = saved;
    }

    // Try syncing with Django backend if user is logged in
    if (getAccessToken()) {
      authAPI.getPreferences()
        .then(prefs => {
          if (prefs.language && (prefs.language === 'en' || prefs.language === 'fa')) {
            setLanguageState(prefs.language);
            localStorage.setItem('app_lang', prefs.language);
            document.documentElement.dir = prefs.language === 'fa' ? 'rtl' : 'ltr';
            document.documentElement.lang = prefs.language;
          }
        })
        .catch(() => {});
    }

    const handleLogout = () => {
      setLanguageState('en');
      localStorage.removeItem('app_lang');
      document.documentElement.dir = 'ltr';
      document.documentElement.lang = 'en';
    };

    const handleLogin = () => {
      authAPI.getPreferences()
        .then(prefs => {
          if (prefs.language && (prefs.language === 'en' || prefs.language === 'fa')) {
            setLanguageState(prefs.language);
            localStorage.setItem('app_lang', prefs.language);
            document.documentElement.dir = prefs.language === 'fa' ? 'rtl' : 'ltr';
            document.documentElement.lang = prefs.language;
          }
        })
        .catch(() => {});
    };

    window.addEventListener('auth_logout', handleLogout);
    window.addEventListener('auth_login', handleLogin);
    return () => {
      window.removeEventListener('auth_logout', handleLogout);
      window.removeEventListener('auth_login', handleLogin);
    };
  }, []);

  const setLanguage = (lang: Language) => {
    setLanguageState(lang);
    localStorage.setItem('app_lang', lang);
    document.documentElement.dir = lang === 'fa' ? 'rtl' : 'ltr';
    document.documentElement.lang = lang;

    // Sync to Django UserPreferences if authenticated
    if (getAccessToken()) {
      authAPI.updatePreferences({ language: lang }).catch(() => {});
    }
  };

  return (
    <LanguageContext.Provider value={{
      language,
      setLanguage,
      t: dictionary[language],
      isRtl: language === 'fa'
    }}>
      {children}
    </LanguageContext.Provider>
  );
};

export const useLanguage = () => {
  const context = useContext(LanguageContext);
  if (!context) throw new Error('useLanguage must be used inside LanguageProvider');
  return context;
};
