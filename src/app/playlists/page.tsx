          ...
          {playlists.map(pl => {
            ...
            return (
              <div key={pl.id} className="p-5 bg-neutral-900 border border-neutral-800 rounded-xl shadow-md flex flex-col justify-between">
                <div>
                  <div className="flex justify-between items-center border-b border-neutral-800 pb-3 mb-3">
                    ...
                    <div className="flex items-center space-x-2 flex-shrink-0">
                      <button onClick={() => handleRename(pl.id, pl.name)} title="Rename Playlist" className="p-1.5 bg-neutral-800 hover:bg-neutral-700 text-neutral-300 rounded text-xs">✏️ Rename</button>
                      <button onClick={() => handleDelete(pl.id)} title="Delete Playlist" className="p-1.5 bg-red-600/20 hover:bg-red-600/40 text-red-400 rounded text-xs">🗑️ Delete</button>
                    </div>
                  </div>

                  <div className="space-y-2 mb-4 max-h-48 overflow-y-auto">
                    {plTracks.length === 0 ? (
                      <p className="text-xs text-neutral-500 py-4 text-center">No songs in this list yet.</p>
                    ) : (
                      plTracks.map(t => (
                        <div key={t.id} className="flex items-center justify-between text-xs bg-black/40 p-2.5 rounded border border-neutral-800/60">
                          <div className="truncate pr-2 flex items-center">
                            <span className="font-bold text-white mr-1.5">{t.title}</span>
                            <span className="text-neutral-600">•</span>
                            <Link href={`/artist/${t.artistId}`} className="text-neutral-400 hover:text-white hover:underline mx-1.5">{t.artistName}</Link>
                            {t.albumId && (
                              <>
                                <span className="text-neutral-600">•</span>
                                {/* Strictly plural /albums/${t.albumId} */}
                                <Link href={`/albums/${t.albumId}`} className="text-neutral-400 hover:text-white hover:underline ml-1.5 truncate max-w-[100px]">{t.album}</Link>
                              </>
                            )}
                          </div>
                          <div className="flex items-center space-x-3 flex-shrink-0">
                            ...
                            <button onClick={() => removeTrackFromPlaylist(pl.id, t.id)} title="Remove from Playlist" className="text-neutral-500 hover:text-red-400">✕</button>
                          </div>
                        </div>
                      ))
                    )}
                ...
                >
                  <span>➕ Add Songs from Archive</span>
                </button>
              </div>
            );
          })}
