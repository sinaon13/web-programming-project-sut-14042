      ...
      <div className="p-6 bg-neutral-900 border border-neutral-800 rounded-xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="text" placeholder="e.g., Audio buffering error..." value={subject} onChange={e => setSubject(e.target.value)} required className="w-full p-2.5 bg-neutral-800 border border-neutral-700 rounded text-sm text-white" />
          </div>
          <div>
            <label className="block text-xs font-semibold text-neutral-400 mb-1">Message Description</label>
            <textarea rows={3} placeholder="Describe your issue in detail..." value={message} onChange={e => setMessage(e.target.value)} required className="w-full p-2.5 bg-neutral-800 border border-neutral-700 rounded text-sm text-white" />
          </div>
          <button type="submit" className="px-6 py-2.5 bg-green-500 hover:bg-green-400 text-black font-bold rounded text-sm transition">Submit Ticket</button>
        </form>
      </div>
            ...
            <div key={t.id} className="p-5 bg-neutral-900 border border-neutral-800 rounded-xl space-y-3 shadow-md">
              <div className="flex justify-between items-center border-b border-neutral-800 pb-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${t.status === 'OPEN' ? 'bg-amber-500/20 text-amber-400' : 'bg-green-500/20 text-green-400'}`}>{t.status}</span>
                </div>
              </div>
